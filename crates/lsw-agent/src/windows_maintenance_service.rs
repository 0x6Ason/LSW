...
//! loopback request. This keeps privileged code out of the normal agent path.
...
use super::{run_maintenance_helper, Configuration};

const SERVICE_NAME: [u16; 21] = [
    b'L' as u16,
    b'S' as u16,
    b'W' as u16,
    b'M' as u16,
...
pub(super) fn run(configuration: Configuration) -> Result<(), Box<dyn std::error::Error>> {
    *CONFIGURATION
        .lock()
        .map_err(|_| "maintenance service configuration lock was poisoned")? = Some(configuration);
    *SERVICE_ERROR
        .lock()
        .map_err(|_| "maintenance service error lock was poisoned")? = None;
    ...
    if unsafe { StartServiceCtrlDispatcherW(service_table.as_ptr()) } == 0 {
        return Err(format!(
            "could not connect LSWMaintenanceHelper to SCM: {}",
    ...
    match SERVICE_ERROR
        .lock()
        .map_err(|_| "maintenance service error lock was poisoned")?
        .take()
    {
...
fn service_main_inner() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let configuration = CONFIGURATION
        .lock()
        .map_err(|_| "maintenance service configuration lock was poisoned")?
        .take()
        .ok_or("maintenance service configuration was not initialized")?;
    let result = run_maintenance_helper(configuration, || {
