...
//! Demand-start SCM adapter for privileged Windows activation operations.
//!
//! This service deliberately exposes no stop control and exits after one
//! authenticated loopback request, limiting the lifetime of LocalSystem code.
...
use super::{run_license_helper, Configuration};

const SERVICE_NAME: [u16; 17] = [
    b'L' as u16,
    b'S' as u16,
    b'W' as u16,
    b'L' as u16,
...
pub(super) fn run(configuration: Configuration) -> Result<(), Box<dyn std::error::Error>> {
    *CONFIGURATION
        .lock()
        .map_err(|_| "license service configuration lock was poisoned")? = Some(configuration);
    *SERVICE_ERROR
        .lock()
        .map_err(|_| "license service error lock was poisoned")? = None;
    ...
    if unsafe { StartServiceCtrlDispatcherW(service_table.as_ptr()) } == 0 {
        return Err(format!(
            "could not connect LSWLicenseHelper to SCM: {}",
    ...
    match SERVICE_ERROR
        .lock()
        .map_err(|_| "license service error lock was poisoned")?
        .take()
    {
...
fn service_main_inner() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let configuration = CONFIGURATION
        .lock()
        .map_err(|_| "license service configuration lock was poisoned")?
        .take()
        .ok_or("license service configuration was not initialized")?;
    let result = run_license_helper(configuration, || {
