...
`1.0.0-beta.3` 是可供工程驗證的 Linux x86_64 beta，不是宣稱所有目標硬體都已通過
...
- 受控 session loopback E2E：stdin close 正常 EOF、authenticated cancel 回傳 130、
  disconnect 釋放 direct child、malformed frame/authentication rejection 及 legacy half-close
...
- Rust 1.76 Linux native build、Windows GNU PE32+ cross-build、unit tests、rustfmt、clippy、
  ...
  及 timeout-bounded headless QEMU smoke gate
...
一般 filesystem Unix socket。QEMU／OVMF／swtpm 原本未預裝，但以上述暫存 Ubuntu
官方套件完成了 firmware-level TCG smoke；產品仍使用的 QMP/VNC/swtpm filesystem
socket 路徑及以下 Windows 工作負載不能在此環境宣稱實機通過：
...
- TCP QMP 的 `stop`/`cont`/`quit` 已對真實 QEMU 驗證；`lswd` 透過 filesystem QMP socket
  管理 Windows workload 的 suspend/resume、graceful/forced stop 與 vTPM soak 尚未驗證
...
- `session-control-v1` 能區分 stdin EOF、authenticated cancel 及啟用選項後的 disconnect，
  但 beta.3 只終止 direct child。尚未用 Windows Job Object／host process group 清理完整
  descendant tree，也沒有 lease/heartbeat 來為 half-open TCP peer 設定有界回收時間；
  descendant 若繼承 output handle，仍可能延後 session 結束。
...
4. 完成 OOBE/first logon，測 ConPTY shell（Unicode、Ctrl、resize、stdin EOF、取消與
   斷線）、exit-code propagation、descendant cleanup、1 GiB file transfer 及並行命令。
