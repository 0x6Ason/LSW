...
unchanged. The fifth migration split the former 1,153-line installation-seed
module into bounded orchestration, unattended answer/password generation, and
guest setup/SCM payload generation while preserving the generated bytes and
public builder API. The final source-layout migration split CLI and daemon
entry points, agent helpers/protocol/session/GUI ownership, Windows capture and
input, ISO resolver/download, supervisor runtime, and Windows E2E orchestration
into bounded modules. All temporary source-size exceptions are removed;
executable entry points are below 300 lines and every production or automation
source is below 1,000 lines.
...
a strict 1,000-line limit to Rust and automation sources, a 300-line limit to
executable entry points, and rejects any reintroduced exception. Moving code
without a single-responsibility boundary does not count as completion.
