...
guest_user_sid=$("$lsw" exec "$instance" -- powershell.exe -NoLogo -NoProfile -Command \
    '[Console]::Out.Write([System.Security.Principal.WindowsIdentity]::GetCurrent().User.Value)')
if ! printf '%s\n' "$guest_user_sid" |
    grep -E '^S-1-[0-9]+(-[0-9]+)+$' >/dev/null
then
    echo "error: guest did not report a valid interactive user SID" >&2
    exit 1
fi
...
password_output=$("$lsw" exec "$instance" -- powershell.exe -NoLogo -NoProfile -Command \
    '$Sid=[System.Security.Principal.WindowsIdentity]::GetCurrent().User.Value; $Account=Get-LocalUser -SID $Sid; if ($null -eq $Account -or -not $Account.PasswordRequired) { exit 41 }; $Winlogon=Get-ItemProperty -LiteralPath "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon"; if ([string]$Winlogon.AutoAdminLogon -eq "1") { exit 42 }; $StoredPassword=$Winlogon.PSObject.Properties["DefaultPassword"]; if ($null -ne $StoredPassword -and -not [string]::IsNullOrEmpty([string]$StoredPassword.Value)) { exit 43 }; [Console]::Out.Write("LSW_WINDOWS_KVM_ACCOUNT_SECURE")')
password_status=$?
set -e
if [ "$password_status" -ne 0 ] || [ "$password_output" != "$password_marker" ]; then
...
# authenticates successfully. PowerShell expands its own variables here.
# shellcheck disable=SC2016
blank_password_output=$("$lsw" exec "$instance" -- powershell.exe -NoLogo -NoProfile -Command \
    '$ErrorActionPreference="Stop"; $Sid=[System.Security.Principal.WindowsIdentity]::GetCurrent().User.Value; $Account=Get-LocalUser -SID $Sid; $Definition="[System.Runtime.InteropServices.DllImport(`"advapi32.dll`", EntryPoint=`"LogonUserW`", ExactSpelling=true, CharSet=System.Runtime.InteropServices.CharSet.Unicode, SetLastError=true)] public static extern bool LogonUserW(string user, string domain, string password, int logonType, int provider, out System.IntPtr token); [System.Runtime.InteropServices.DllImport(`"kernel32.dll`", SetLastError=true)] public static extern bool CloseHandle(System.IntPtr handle);"; $Native=Add-Type -MemberDefinition $Definition -Name NativeLogon -Namespace LswE2e -PassThru; $Token=[IntPtr]::Zero; if ($Native::LogonUserW($Account.Name, ".", "", 2, 0, [ref]$Token)) { [void]$Native::CloseHandle($Token); exit 44 }; if ([System.Runtime.InteropServices.Marshal]::GetLastWin32Error() -ne 1326) { exit 45 }; [Console]::Out.Write("LSW_WINDOWS_KVM_BLANK_PASSWORD_REJECTED")')
blank_password_status=$?
set -e
if [ "$blank_password_status" -ne 0 ] \
    || [ "$blank_password_output" != "$blank_password_marker" ]
then
...
# shellcheck disable=SC2016
conpty_output=$(
    printf "\$a='%s'; \$b='%s'; Write-Output (\$a+\$b)\rexit\r" \
...
cold_user_sid=$("$lsw" exec "$instance" -- powershell.exe -NoLogo -NoProfile -Command \
    '[Console]::Out.Write([System.Security.Principal.WindowsIdentity]::GetCurrent().User.Value)')
if [ "$cold_user_sid" != "$guest_user_sid" ]; then
    echo "error: cold-started agent did not return in the original interactive user context" >&2
    exit 1
fi
assert_daemon_alive
...
echo "Windows/KVM E2E passed: Setup -> OOBE -> agent -> ConPTY -> shutdown -> cold restart -> cleanup."
